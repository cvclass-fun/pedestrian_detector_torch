...
%
% INPUTS
%  varargin:[1] - dataset path
%           [2] - experiment id. If set to 0, all experiments are
%           considered for testing.
%           [3] - Total number of algorithms to be plotted.
%           [4] - select a dataset for testing by its id. If set to 0, all datasets are considered
%           for testing.
%           [5] - path to the algorithms results. If empty, it will
%           consider the path is inside the dataset path under the subfolder
%           'algorithms'.
%           [6] - Folder path to store plot results.
%           [7] - String with input algorithms to plot. 
%           [8] - Flag to indicate what algorithms to plot. 
%           If > 0, plots each alg in individual graphs.
%           If == 0 plots all algorithms plus input algorithms in a single graph.
%           If <0, plots only input algorithms in a single graph.
%
% OUTPUTS
%
% EXAMPLE
...
[experimentID, AlgPlotNum, dataNamesID, pathDataset, pathAlgs, plotName, algorithmsNames, flag_benchmark] = parseInputs(varargin);

%% add input algoritghms to the list
if ~isempty(algorithmsNames)
    struct_size = size(algs,1);
    for i=1:1:size(algorithmsNames,2)
        algs(struct_size+i).name = algorithmsNames{1,i};
        algs(struct_size+i).resize = 0;
        algs(struct_size+i).color = clrs(size(algs,1)+i,:);
        if rem(i,2)~=0    
            algs(struct_size+i).style = '-';
        else
            algs(struct_size+i).style = '--';
        end
    end
...
if dataNamesID > 0, dataNames = dataNames(dataNamesID); else dataNames = dataNames(:); end % select one or more databases for evaluation
if experimentID > 0, exps = exps(experimentID); else  exps = exps(:); end % select one or more experiment for evaluation
algs = algs(:);
...
plotAlg = flag_benchmark; %0; % if true one plot per alg else one plot per exp
plotNum = AlgPlotNum;     % only show best plotNum curves (and VJ and HOG)
...
for d=1:length(dataNames), dataName=dataNames{d};
  ...
  for i=1:n, keep(i)=exist([pathAlgs names{i} set],'dir'); end
  algs=algs0(keep);
  ...
  dts = loadDt( algs, [plotName '/cache/'], aspectRatio, dataName, pathDataset, pathAlgs);
  gts = loadGt( exps, [plotName '/cache/'], aspectRatio, bnds, dataName, pathDataset );
  res = evalAlgs( [plotName '/cache/'], algs, exps, gts, dts );
  
  % plot curves and bbs
  plotExps( res, plotRoc, plotAlg, plotNum, [plotName '/result'], algorithmsNames, ...
    samples, lims, reshape([algs.color]',3,[])', {algs.style} );
  plotBbs( res, [plotName '/result'], bbsShow, bbsType, dataName, pathDataset );
end
...
function [experimentID, AlgPlotNum, dataNamesID, pathDataset, pathAlgs, plotName, algorithmsNames, flag_benchmark] = parseInputs(input_varargin)
% validate input arguments

% num arguments
nargs = size(input_varargin,2);

% arg1
if (nargs>=1), 
    if ~isempty(input_varargin{1}),
        assert(input_varargin{1}>=0 && input_varargin{1}<=18)
        experimentID = input_varargin{1}; 
    else experimentID = 1; end
else experimentID = 1; end

% arg2
if (nargs>=2),
    if~isempty(input_varargin{2}), 
        assert(input_varargin{2}>=0 && input_varargin{2}<=78)
        AlgPlotNum = input_varargin{2}; 
    else AlgPlotNum = 15; end
else AlgPlotNum = 15; end

% arg3
if (nargs>=3) 
    if ~isempty(input_varargin{3}),
        assert(input_varargin{3}>=0 && input_varargin{3}<=7)
        dataNamesID = input_varargin{3}; 
    else dataNamesID = 1; end
else dataNamesID = 1; end

% arg4
if (nargs>=4) 
    if ~isempty(input_varargin{4}), 
        pathDataset = input_varargin{4};
        if pathDataset(end) ~= '/', pathDataset = [pathDataset '/']; end
        assert(exist(pathDataset, 'dir')== 7, 'Must insert a valid path for the algorithms used for evaluation comparison.')
    else assert(false, 'Must insert a valid path for the algorithms results.'); end
else
    pathDataset = './';
end
...
% arg5
if (nargs>=5) 
    if ~isempty(input_varargin{5}), 
        pathAlgs = input_varargin{5};
        if pathAlgs(end) ~= '/', pathAlgs = [pathAlgs '/']; end
        assert(exist(pathAlgs, 'dir')== 7, 'Must insert a valid path for the algorithms used for evaluation comparison.')
    else assert(false, 'Must insert a valid path for the algorithms results.'); end
else
    pathAlgs = '/algorithms/';
end
...
% arg6
if (nargs>=6) 
    if ~isempty(input_varargin{6}),
        plotName = input_varargin{6};
    else plotName = '/plots/'; end
else
    plotName = '/plots/';
end
% name for all plots (and also temp directory for results)  
if(~exist(plotName,'dir')), mkdir(plotName); end
if(~exist([plotName '/cache/'],'dir')), mkdir([plotName '/cache/']); end

% arg7
if (nargs>=7) 
    if ~isempty(input_varargin{7}),
        % split algorithms string
        string_algs = input_varargin{7};
        % remove whitespaces
        string_algs(ismember(string_algs,' .:;!')) = [];
        % split strings by commas 
        tmpstr = strsplit(string_algs,',');
        % append all string names into a single cell array
        algorithmsNames = {};
        for i=1:1:size(tmpstr,2)
            algorithmsNames{i} = tmpstr{1,i};
        end
    else algorithmsNames = ''; end
else
    algorithmsNames = '';
end
...
% arg8
if (nargs>=8) 
    if ~isempty(input_varargin{8}),
        flag_benchmark = input_varargin{8};
    else flag_benchmark = 1; end
else
    flag_benchmark = 1;
end
...
function plotExps( res, plotRoc, plotAlg, plotNum, plotName, algorithmsNames,...
...
for p=1:nPlots
  ...
  elseif ( plotAlg == 0 )
    % plot Top-N algorithms 
    xs1=xs(p,:); ys1=ys(p,:); fName1=[fName stre{p}]; lgd1=stra;
    for d=1:nDt, lgd1{d}=sprintf('%.2f%% %s',scores1(p,d),stra{d}); end
    kp=[find(strcmp(stra,'VJ')) find(strcmp(stra,'HOG'))];
    for i=1:1:size(algorithmsNames,2)
        kp = [kp find(strcmp(stra,algorithmsNames{1,i}))];
    end
    kp = [kp 1 1];
    [~,ord]=sort(scores(p,:)); 
    tmp = ord==-1; for i=1:1:(2+size(algorithmsNames,2)), tmp=tmp|ord==kp(i); end
    kp=tmp;
    j=find(cumsum(~kp)>=plotNum-(2+size(algorithmsNames,2))); kp(1:j(1))=1; ord=fliplr(ord(kp));
    ...
    % plot input algorithms only
    if size(stre,1)<p, break; end
    xs1=xs(p,:); ys1=ys(p,:); fName1=[fName stre{p}]; lgd1=stra;
    for d=1:nDt, lgd1{d}=sprintf('%.2f%% %s',scores1(p,d),stra{d}); end
    kp=[];
    for i=1:1:size(algorithmsNames,2)
    ...
    [~,ord]=sort(scores(p,:));
    tmp = ord==-1; for i=1:1:size(algorithmsNames,2), tmp=tmp|ord==kp(i); end
    kp=tmp;
    ord=fliplr(ord(kp));
    xs1=xs1(ord); ys1=ys1(ord); lgd1=lgd1(ord); colors1=colors(ord,:);
    styles1=styles(ord); f=fopen([fName1 '.txt'],'w');
  ...
  %pdollar_savefig(fName1,1,'pdf','-r300','-fonts'); close(1);
  export_fig(fName1,'-transparent','-native','-a1'); close(1);
...
function plotBbs( res, plotName, pPage, type, dataName, pathDataset )
...
[~,setIds,vidIds,skip]=dbInfo(dataName, pathDataset);
k=length(res(1).gtr); is=zeros(k,3); k=0;
for s=1:length(setIds)
  for v=1:length(vidIds{s})
    A=loadVbb(s,v,dataName, pathDataset); s1=setIds(s); v1=vidIds{s}(v);
...
for page=1:min(pPage,ceil(n/mRows/nCols))
  ...
  %pdollar_savefig([fName int2str2(page-1,2)],h,'png','-r200','-fonts'); close(h);
  figure(h); export_fig([fName int2str2(page-1,2)],'-transparent','-native','-a1'); close(h);
...
function A = loadVbb( s, v, dataName, pathDataset )
% Load given annotation (caches AS for speed).
persistent AS pth sIds vIds; [pth1,sIds1,vIds1]=dbInfo(dataName, pathDataset);
if(~strcmp(pth,pth1) || ~isequal(sIds,sIds1) || ~isequal(vIds,vIds1))
  [pth,sIds,vIds]=dbInfo(dataName, pathDataset); AS=cell(length(sIds),1e3); end
...
function gts = loadGt( exps, plotName, aspectRatio, bnds, dataName, pathDataset )
...
for i=1:nExp
  ...
  for s=1:length(setIds)
    for v=1:length(vidIds{s})
      A = loadVbb(s,v,dataName, pathDataset);
...
function dts = loadDt( algs, plotName, aspectRatio, dataName, pathDataset, pathAlgs )
