...
function options.parse(arg)
    ...
    cmd:text()
    cmd:option('-expID',    'alexnet_vanilla_frcnn', 'Experiment ID')
    ...
    cmd:option('-proposalAlg',     'acf', 'ROI proposal generator. Options: acf, ldcf or edgeboxes.')
    ...
    cmd:option('-clsType',      'parallel', 'Classifier network. Options: simple (original frcnn) | concat | parallel.')
    cmd:option('-featID',              2, 'Feature net architecture ID. Warning: number of feature networks vary from net type.')
