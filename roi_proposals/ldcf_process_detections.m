function [ boxes ] = ldcf_process_detections(paths, skip_step, model, setType)
...
%% Process edgeboxes
fprintf('\n==> Compute LDCF boxes:\n')
boxes = process_ldcf_images(filenames, model, setType);
end
...
function [boxes] = process_ldcf_images(fname, model, setType)
%% setup detector
detector=model;
...
progressbar = textprogressbar(size(fname_split,1), 'barlength', 20, ...
                         'updatestep', 1, ...
                         'startmsg', sprintf('Processing LDCF boxes %s: files=%d, batchSize=%d ', setType, nFiles, batchSize),...
                         'endmsg', ' Done!', ...
                         'showbar', true, ...
                         'showremtime', true, ...
                         'showactualnum', true, ...
