function script_process_rois_ldcf_ETH(varargin)
%% process ACF ROI boxes for the ETH dataset
% input arguments:
%     [1] - threshold for suppression of weak(er) boxes
%
fprintf('\n*********************************************************')
fprintf('\n**** Start ETH LDCF roi detection/extraction script. ****')
fprintf('\n*********************************************************')
...
%% load options
[model] = ldcf_options_process('eth');
...
%% script complete
fprintf('\n-------------------------------------------')
fprintf('\nETH LDCF boxes processing script completed.')
fprintf('\n-------------------------------------------\n')
end